...
#' @param level An integer. From which level in the GO hierarchy the terms should be used for the enrichment. 
#'  To see which term resides on which level, please refer to the browse section of AmiGO2 (http://amigo.geneontology.org/amigo/landing).
#' @param top Integer. The number of most strongly enriched terms to return per cluster.
#' @param ont The subontology to use, three options are available: the molecular functions describing the actions a gene is involved in on a molecular level (“MF”), 
#'  cellular component (“CC”), which describes the location within the cell at which a gene exerts its function and biological process (“BP”) i.e., the overall procedure or program a gene is involved in.
#' @param fast A Boolean.  If set to TRUE, the enrichment analysis is based on the GO file saved in “supplementary_data” instead of accessing the entire ontology. 
#'  In this case, the “level” parameter is ignored. This is much faster at the cost of lower resolution. 
#'  If the parameter is set to FALSE (default), the entire database can be searched at any level. 
#'  The reference gene list for all enrichments is the union of all genes present in the count files provided for the CoCena analysis.
...
go_profiling <- function(level, top, ont, fast = F, padj = "BH", clusters = c("all"), qval = 0.05){
  ...
  if(fast){
    for(c in clusters){
      ...
      enrich <- clusterProfiler::enricher(gene = genes, 
                                          TERM2GENE = hcobject[["supplementary_data"]][["go"]],
                                          qvalueCutoff = qval, 
                                          pAdjustMethod = padj, 
                                          universe = universe)
    ...
    for(c in clusters){
      ...
      }else if(base::tolower(hcobject[["global_settings"]][["organism"]]) == "human"){
      ...
      if(!base::is.null(enrich)){
        tmp <- clusterProfiler::gofilter(enrich, level = level)@result
        tmp <- dplyr::filter(tmp, qvalue <= qval)
        if(base::nrow(tmp) == 0){
          next
        }else{
          ...
          res[[c]] <- clusterProfiler::gofilter(enrich, level = level)@result
        }
      }
    }
  ...
  openxlsx::write.xlsx(x = res, file = base::paste0(hcobject[["working_directory"]][["dir_output"]], hcobject[["global_settings"]][["save_folder"]], "/go_enrichment.xlsx"), overwrite = T)
