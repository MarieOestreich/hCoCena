...
#' @param setnum An integer. The number of the dataset for which the plot should be  created.
#' @param group_col A string. The name of the column in the annotation of datset 'setnum' that contains your groups (e.g. conditions) that are to be plotted along the x-axis.
#' @param meta_col A string. The name of the column in the annotation of datset 'setnum' that contains the information that is to be inspected (e.g. outcome or age).
#' @param type Either "cat", if "meta_col" is categorical, or "num", if "meta_col" is numerical.
#' @export


meta_plot <- function(setnum, group_col = NULL, meta_col = NULL, type = "cat"){

  anno <- hcobject[["data"]][[base::paste0("set", setnum, "_anno")]]
