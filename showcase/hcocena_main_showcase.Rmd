...
## OPTIONAL: Define colors for meta data annotations
  This chunk in the satellite markdown allows you to define color vectors to be used e.g. for heatmap annotations. If you don't define them explicitely, implemented colors will be used for visualization. 
...
## Database enrichment
  ...
  Enrichment analysis of the found modules using the GO, KEGG, Hallmark and Reactome databases. For detailed information on parameter settings, please run ?hcocena::functional_enrichment.
  
  In our showcase, we are interested in getting the GO- and HALLMARK enrichment for selected clusters of interest (we selected them because of their stimulus-specific activation patterns). In both cases, we want to retrieve the top 5 most enriched terms (may be less if less than 5 terms are enriched). We also filter the results for those with adjusted p-values not higher than 0.1. 
...
TF_overrep(clusters = c("lightblue", "darkgrey"),
           topTF = 5, 
           topTarget = 5)
