...
author: "Marie Oestreich, Lisa Holsten"
date: "19 10 2023"
output: html_document
---

This markdown contains optional functions which can be used to expand the mandatory hcocena workflow functions displayed in the hcocena_main markdown script.
...
plot_sample_distributions(plot_type = "freqdist",
                          log_2 = F, 
                          plot = T)
...
```{r meta data plots}
# For the array data
meta_plot(set = 1, group_col = "merged", meta_col = "stimuli", type = "cat", cols = c("IL4" = "#00A1D5FF", "IFNg" = "#79AF97FF", "baseline" = "#374E55FF"))
meta_plot(set = 1, group_col = "merged", meta_col = "Donor", type = "cat", cols = c("BC9" = "#FBB4AE", "BC11" = "#B3CDE3", "BC13" = "#CCEBC5", "BC78" = "#DECBE4"))

# For the RNA-seq data
meta_plot(set = 2, group_col = "merged", meta_col = "Phenotype", type = "cat", cols = c("IL4" = "#00A1D5FF", "IFNg" = "#79AF97FF", "baseline" = "#374E55FF"))
meta_plot(set = 2, group_col = "merged", meta_col = "Donor", type = "cat", cols = c("BC9" = "#FBB4AE", "BC11" = "#B3CDE3", "BC13" = "#CCEBC5", "BC78" = "#DECBE4"))
...
## Transcription factor analysis for the entire network
  ...
  To perform a transcription factor enrichment analysis based on the entire network you can run the following function.You can specify the number of TFs and their targets outputed to hcobject[["integrated_output"]][["enrichall"]].
...
find_hubs(clusters = c("steelblue", "gold", "maroon", "plum"),
          top = 10, 
          tree_layout = F, 
          TF_only = "TF",
          save = T, 
...
highlight_geneset(gene_set = c("STAT6", "MAFG", "ETS2", "RUNX1", "TFE3", "HES6", "IRF4", "FOXQ1",
                               "CREB5", "SPN", "SOX8", "SIP1", "FOXD2", "GTF3C3", "BATF3", "EGR3", "FOS"), 
                   name = "Geneset_IL4")
...
### User Specific Cluster Profiling
  ...
  Offers a user-defined enrichment analysis for the identified modules. There are two options:
	...
	2) Instead of providing an enrichment file, the user can also choose a database - so far Gene Ontology and KEGG are supported – and define a vector of keys as strings. The function then conducts an enrichment analysis using clusterProfiler functions (Guangchuang Yu, 2012) and filters the enrichment results for terms including the defined keys. In this case, the "from_file" parameter needs to be set to FALSE and the "enrichment_keys" parameter must be set to the vector of keys, the modules should be screened for. By setting the parameter 'db = "GO"' or 'db = "KEGG"', the GO or KEGG database is chosen accordingly. Up to 2 user-defined enrichments are possible so far and they are saved as "enriched_per_cluster" and "enriched_per_cluster2" in hcobject[["satellite_outputs"]]. 
...
```{r enriched signatures, echo = F, fig.height=6, fig.width=12}

user_specific_cluster_profiling(from_file =  T, path = paste0(hcobject$working_directory$dir_reference_files, "immune_sig_m.csv"))

user_specific_cluster_profiling(from_file = F, enrichment_keys = c("interferon-gamma", "interleukin-4"), db = "GO", qval = 0.5)
...
```{r create column annotation matrices, fig.height=6, fig.width=12}

# some categorical annotations
col_anno_categorical(variables = c("stimuli", "Phenotype"), variable_label = "Stimulation") 
col_anno_categorical(variables = c("Donor", "Donor"), variable_label = "Donor")
...
meta_correlation_cat(set = 1, meta = "stimuli", p_val = 0.1)
meta_correlation_cat(set = 2, meta = "Phenotype", p_val = 0.1)
